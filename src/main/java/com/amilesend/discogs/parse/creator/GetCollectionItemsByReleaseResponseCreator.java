 ...
 * resource type so that method operations can be performed on the {@link GetCollectionItemsByReleaseResponse}
 * resource.
 *
 * @see InstanceCreator
...
public class GetCollectionItemsByReleaseResponseCreator
        implements InstanceCreator<GetCollectionItemsByReleaseResponse> {
    /** The current client connection instance. */
    private final DiscogsConnection connection;

    @Override
    public GetCollectionItemsByReleaseResponse createInstance(final Type type) {
        return GetCollectionItemsByReleaseResponse.builder().connection(connection).build();
    }
}
