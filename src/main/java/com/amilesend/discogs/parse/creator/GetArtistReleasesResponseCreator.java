 ...
 * resource type so that method operations can be performed on the {@link GetArtistReleasesResponse}
 * resource.
 *
 * @see InstanceCreator
...
public class GetArtistReleasesResponseCreator implements InstanceCreator<GetArtistReleasesResponse> {
    /** The current client connection instance. */
    private final DiscogsConnection connection;

    @Override
    public GetArtistReleasesResponse createInstance(final Type type) {
        return GetArtistReleasesResponse.builder().connection(connection).build();
    }
}