 ...
 * resource type so that method operations can be performed on the {@link GetExportsResponse}
 * resource.
 *
 * @see InstanceCreator
...
public class GetExportsResponseCreator implements InstanceCreator<GetExportsResponse> {
    /** The current client connection instance. */
    private final DiscogsConnection connection;

    @Override
    public GetExportsResponse createInstance(final Type type) {
        return GetExportsResponse.builder().connection(connection).build();
    }
}
