 ...
 * resource type so that method operations can be performed on the {@link SearchResponse}
 * resource.
 *
 * @see InstanceCreator
 * @see SearchResponse
...
public class SearchResponseCreator implements InstanceCreator<SearchResponse> {
    /** The current client connection instance. */
    private final DiscogsConnection connection;

    @Override
    public SearchResponse createInstance(final Type type) {
        return SearchResponse.builder().connection(connection).build();
    }
}