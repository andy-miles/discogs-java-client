 ...
 * resource type so that method operations can be performed on the {@link GetUserContributionsResponse}
 * resource.
 *
 * @see InstanceCreator
...
public class GetUserContributionsResponseCreator implements InstanceCreator<GetUserContributionsResponse> {
    /** The current client connection instance. */
    private final DiscogsConnection connection;

    @Override
    public GetUserContributionsResponse createInstance(final Type type) {
        return GetUserContributionsResponse.builder().connection(connection).build();
    }
}
