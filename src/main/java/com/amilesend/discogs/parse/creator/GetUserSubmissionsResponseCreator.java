 ...
 * resource type so that method operations can be performed on the {@link GetUserSubmissionsResponse} resource.
 *
 * @see InstanceCreator
 * @see GetUserSubmissionsResponse
 */
@RequiredArgsConstructor
public class GetUserSubmissionsResponseCreator implements InstanceCreator<GetUserSubmissionsResponse> {
    /** The current client connection instance. */
    private final DiscogsConnection connection;

    @Override
    public GetUserSubmissionsResponse createInstance(final Type type) {
        return GetUserSubmissionsResponse.builder().connection(connection).build();
    }
}
