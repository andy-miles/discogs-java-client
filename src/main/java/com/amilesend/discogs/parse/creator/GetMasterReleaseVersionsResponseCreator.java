 ...
 * resource type so that method operations can be performed on the {@link GetMasterReleaseVersionsResponse}
 * resource.
 *
 * @see InstanceCreator
...
public class GetMasterReleaseVersionsResponseCreator implements InstanceCreator<GetMasterReleaseVersionsResponse> {
    /** The current client connection instance. */
    private final DiscogsConnection connection;

    @Override
    public GetMasterReleaseVersionsResponse createInstance(final Type type) {
        return GetMasterReleaseVersionsResponse.builder().connection(connection).build();
    }
}