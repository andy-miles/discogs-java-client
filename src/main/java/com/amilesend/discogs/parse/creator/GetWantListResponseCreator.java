 ...
 * resource type so that method operations can be performed on the {@link GetWantListResponse} resource.
 *
 * @see InstanceCreator
 * @see GetWantListResponse
 */
@RequiredArgsConstructor
public class GetWantListResponseCreator implements InstanceCreator<GetWantListResponse> {
    /** The current client connection instance. */
    private final DiscogsConnection connection;

    @Override
    public GetWantListResponse createInstance(final Type type) {
        return GetWantListResponse.builder().connection(connection).build();
    }
}
