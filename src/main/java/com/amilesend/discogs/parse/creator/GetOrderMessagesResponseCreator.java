 ...
 * resource type so that method operations can be performed on the {@link GetOrderMessagesResponse} resource.
 *
 * @see InstanceCreator
 * @see GetOrderMessagesResponse
 */
@RequiredArgsConstructor
public class GetOrderMessagesResponseCreator implements InstanceCreator<GetOrderMessagesResponse> {
    /** The current client connection instance. */
    private final DiscogsConnection connection;

    @Override
    public GetOrderMessagesResponse createInstance(Type type) {
        return GetOrderMessagesResponse.builder().connection(connection).build();
    }
}
