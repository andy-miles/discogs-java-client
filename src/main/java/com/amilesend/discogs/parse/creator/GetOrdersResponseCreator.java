 ...
 * resource type so that method operations can be performed on the {@link GetOrdersResponseCreator} resource.
 *
 * @see InstanceCreator
 * @see GetOrdersResponseCreator
 */
@RequiredArgsConstructor
public class GetOrdersResponseCreator implements InstanceCreator<GetOrdersResponse> {
    /** The current client connection instance. */
    private final DiscogsConnection connection;

    @Override
    public GetOrdersResponse createInstance(Type type) {
        return GetOrdersResponse.builder().connection(connection).build();
    }
}
